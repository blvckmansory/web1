...
const PostPreview = ({ id, title, cover, style, className, description }: PostPreviewProps) => (
	<article
		...
		<NavLink
			href={`/posts/${id}`}
			className="group mt-3 text-link-default uppercase flex flex-row gap-x-1 items-center text-sm">
			<span>Подробнее</span>
			<ArrowIcon
				size={18}
